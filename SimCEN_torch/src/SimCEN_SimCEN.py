...
class SimCEN_SimCEN(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="SimCEN_SimCEN",
                 gpu=-1,
                 learning_rate=1e-3,
                 embedding_dim=10,
                 alpha=0.2,
                 cl_temperature=0.2,
                 mlep1_hidden_units=[64],
                 mlep2_hidden_units=[64],
                 ...
                 **kwargs):
        super(SimCEN_SimCEN, self).__init__(feature_map,
                                               model_id=model_id,
                                               gpu=gpu,
        ...
        self.mlep1 = MLEP_Fixed_Dimension(input_dim=flatten_dim * 3,
                                          hidden_units=mlep1_hidden_units,
        ...
        self.mlep2 = MLEP_Dynamic_Dimension(input_dim=flatten_dim * 3,
                                            hidden_units=mlep2_hidden_units,
                                            ego_hidden_activations=ego_hidden_activations,
                                            v1_hidden_activations=v1_hidden_activations,
                                            v2_hidden_activations=v2_hidden_activations,
                                            ego_dropout=ego_dropout,
        ...
        if self.embedding_share:
            ...
            self.embedding_layer1 = FeatureEmbedding(feature_map, embedding_dim)
            self.embedding_layer2 = FeatureEmbedding(feature_map, embedding_dim)
            self.segmentation1 = Segmentation(num_fields=num_fields,
                                              embedding_dim=embedding_dim,
                                              flatten_dim=flatten_dim)
            self.segmentation2 = Segmentation(num_fields=num_fields,
                                              embedding_dim=embedding_dim,
                                              flatten_dim=flatten_dim)

        self.dropout = nn.Dropout(p=through_dropout)
        self.fc = nn.Linear(mlep1_hidden_units[-1] + mlep2_hidden_units[-1], 1, bias=True)
    ...
    def forward(self, inputs):
        ...
        if self.embedding_share:
            mlep_feature_emb = self.embedding_layer(X)
            mlep1_ego, mlep1_view1, mlep1_view2 = self.segmentation(mlep_feature_emb)
            mlep2_ego, mlep2_view1, mlep2_view2 = mlep1_ego, mlep1_view1, mlep1_view2
        else:
            mlep1_feature_emb = self.embedding_layer1(X)
            mlep1_ego, mlep1_view1, mlep1_view2 = self.segmentation1(mlep1_feature_emb)
            mlep2_feature_emb = self.embedding_layer2(X)
            mlep2_ego, mlep2_view1, mlep2_view2 = self.segmentation2(mlep2_feature_emb)

        mlep1_V = torch.cat([mlep1_ego, mlep1_view1, mlep1_view2], dim=-1)
        mlep1_V = self.mlep1(mlep1_V)

        mlep2_V = torch.cat([mlep2_ego, mlep2_view1, mlep2_view2], dim=-1)
        mlep2_V = self.mlep2(mlep2_V)

        mlep1_ego, mlep1_v1, mlep1_v2 = torch.chunk(mlep1_V, chunks=3, dim=-1)
        mlep2_ego, mlep2_v1, mlep2_v2 = torch.chunk(mlep2_V, chunks=3, dim=-1)

        mlep1_ego4v1v2 = self.dropout(mlep1_ego)
        mlep1_v1 = mlep1_ego4v1v2 + mlep1_v1
        mlep1_v2 = mlep1_ego4v1v2 + mlep1_v2

        mlep2_ego4v1v2 = self.dropout(mlep2_ego)
        mlep2_v1 = mlep2_ego4v1v2 + mlep2_v1
        mlep2_v2 = mlep2_ego4v1v2 + mlep2_v2

        y_pred = self.fc(torch.cat([mlep1_V, mlep2_V], dim=-1))
        y_pred = self.output_activation(y_pred)
        return_dict = {"y_pred": y_pred,
                       "mlep1_ego": mlep1_ego, "mlep2_ego": mlep2_ego,
                       "mlep1_v1": mlep1_v1, "mlep2_v1": mlep2_v1,
                       "mlep1_v2": mlep1_v2, "mlep2_v2": mlep2_v2}
        return return_dict
    ...
    def add_loss(self, inputs):
        ...
        mlep1_ego = return_dict["mlep1_ego"]
        mlep1_v1 = return_dict["mlep1_v1"]
        mlep1_v2 = return_dict["mlep1_v2"]
        mlep2_ego = return_dict["mlep2_ego"]
        mlep2_v1 = return_dict["mlep2_v1"]
        mlep2_v2 = return_dict["mlep2_v2"]
        cl_loss = self.InfoNCE(mlep1_ego, mlep1_v1, mlep1_v2, cl_temperature=self.cl_temperature) + \
                  self.InfoNCE(mlep2_ego, mlep2_v1, mlep2_v2, cl_temperature=self.cl_temperature)
...
class MLEP_Fixed_Dimension(nn.Module):
    def __init__(self,
                 input_dim,
                 hidden_units=[],
                 ...
                 v1_batch_norm=False,
                 v2_batch_norm=False):
        super(MLEP_Fixed_Dimension, self).__init__()
...
class Linear_unit(nn.Module):
    def __init__(self, input_dim, output_dim, bias=True):
        ...
        self.gate = nn.Sequential(nn.Linear(one_third, one_third, bias=True),  # 此偏差影响万分之7的性能
                                  nn.Sigmoid())
...
class MLEP_Dynamic_Dimension(nn.Module):
    def __init__(self,
                 input_dim,
                 hidden_units=[],
                 ego_hidden_activations=None,
                 ...
                 v1_batch_norm=False,
                 v2_batch_norm=False):
        super(MLEP_Dynamic_Dimension, self).__init__()
        if type(ego_dropout) != list:
            ego_dropout = [ego_dropout] * len(hidden_units)
        if type(v1_dropout) != list:
            v1_dropout = [v1_dropout] * len(hidden_units)
        ...
        for idx in range(len(hidden_units) - 1):
            one_third = int(hidden_units[idx + 1] / 3)
            self.layer.append(Linear_unit_Dynamic_Dimension(hidden_units[idx], hidden_units[idx + 1]))
            if ego_batch_norm:
                self.norm_ego.append(nn.BatchNorm1d(one_third))
            if v1_batch_norm:
                self.norm_view_1.append(nn.BatchNorm1d(one_third))
...
class Linear_unit_Dynamic_Dimension(nn.Module):
    def __init__(self, input_dim, output_dim, bias=True):
        super(Linear_unit_Dynamic_Dimension, self).__init__()
        assert output_dim % 3 == 0, "output_dim should be divisible by 3."
        self.linear = nn.Linear(input_dim, output_dim, bias=bias)
        one_third = int(output_dim / 3)
        self.gate = nn.Sequential(nn.Linear(one_third, one_third, bias=True),  # 此偏差影响万分之7的性能
    ...
    def forward2(self, V):
        ...
        ego_gate = self.gate(ego) / self.gate_tau
        noise_v1, noise_v2 = torch.chunk(self.noise, chunks=2, dim=-1)
        v1 = ego_gate * v1 + v1 + noise_v1
        v2 = ego_gate * v2 + v2 + noise_v2
        return ego, v1, v2
