...
class SimCEN_MLP(BaseModel):
    def __init__(self,
                 ...
                 **kwargs):
        super(SimCEN_MLP, self).__init__(feature_map,
                                       model_id=model_id,
                                       gpu=gpu,
        ...
        self.mlep = MLEP(input_dim=flatten_dim * 3,
                         hidden_units=mlep_hidden_units,
        ...
        # BN is False On Criteo
        self.mlp = MLP_Block(input_dim=flatten_dim,
                             output_dim=1,
                             hidden_units=mlp_hidden_units,
                             hidden_activations=ego_hidden_activations,
                             output_activation=None,
                             dropout_rates=ego_dropout,
                             batch_norm=ego_batch_norm)

        self.segmentation = Segmentation(num_fields=num_fields,
                                         embedding_dim=embedding_dim,
                                         flatten_dim=flatten_dim)
        self.dropout = nn.Dropout(p=through_dropout)
        self.W = nn.Linear(mlep_hidden_units[-1], 1, bias=True)
